...
//..................Указание дней без содержания................................
if (isset($_POST['start'])){
  $chatid = $_POST['chatid'];
  $start = $_POST['start'];
  $end = $_POST['end'];
  $sql ="INSERT INTO vacation_aprove (chat_id, start, \"end\", status) VALUES ('".$chatid."', '".$start."', '".$end."', 'D') RETURNING id";
  ...
  $getQuery = array(
    "chat_id" 	=> $chatid,
    "text" => "Заявка на день/дни без содержания с ".$start." по ".$end." была отправлена.",
    "parse_mode" => "html"
  );
  ...
  $getQuery = array(
    "chat_id" 	=> $approve, 
    "text"  	=> "[#".$id."]\n".$fullname." хочет взять день/дни без содержания с ".$start." по ".$end,
    'reply_markup' => json_encode(array(
        'inline_keyboard' => array(
            array(
                array(
                    'text' => 'Одобрить',
                    'callback_data' => 'day_aprove',
                ),
                array(
                  'text' => 'Отклонить',
                  'callback_data' => 'day_!aprove',
              ),
            )
        ),
    )),
